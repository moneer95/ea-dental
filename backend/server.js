    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: cartItems.map((item, idx) => {
        ...
        if(item.quantity){
          ...
          price = getProductPrice( item.id, item.choiceId[0]).then(p => {price = p}).catch(error => {price = 1000})
        }
        ...
        if(choices[idx].inStock && !item.quantity){
          ...
          price = getCourseTicketPrice(item.category, item.docID, item.shoppingOption, item.choiceId[0]).then(p => {price = p}).catch(error => {price = 1000})
        }
